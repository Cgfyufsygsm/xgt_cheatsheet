...
\begin{itemize}
  ...
  \item 非负离散：$E(X) = \sum_{x=0}^{+\infty}P(X>x)$，连续 $E(X) = \int_0^{+\infty} P(X>x) \d x$
  \item $\int_{-\infty}^{+\infty} e^{-t^2}\d t = \sqrt\pi$（高斯积分）
